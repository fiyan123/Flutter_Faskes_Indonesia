class BaseUrl {
  
  static String auth = 
      'https://demo-elearning.smkassalaambandung.sch.id/api/login';
  static String headline = 
      'https://gist.githubusercontent.com/olipiskandar/3d4c85f5a29ecf847f0abf3bb00a1c36/raw/ad7fbfae0dd5a8d2c08303f12401a6a2f22bcb52/headline.json';
  static String entertainment = 
      'https://gist.githubusercontent.com/olipiskandar/3d4c85f5a29ecf847f0abf3bb00a1c36/raw/ad7fbfae0dd5a8d2c08303f12401a6a2f22bcb52/entertainment.json';
  static String sports = 
      'https://gist.githubusercontent.com/olipiskandar/3d4c85f5a29ecf847f0abf3bb00a1c36/raw/ad7fbfae0dd5a8d2c08303f12401a6a2f22bcb52/sports.json';
  static String technology = 
      'https://gist.githubusercontent.com/olipiskandar/3d4c85f5a29ecf847f0abf3bb00a1c36/raw/ad7fbfae0dd5a8d2c08303f12401a6a2f22bcb52/technology.json';
  static String covid = 
      'https://kipi.covid19.go.id/api/get-faskes-vaksinasi?skip=0&province=JAWA+BARAT&city=KAB.+BANDUNG';
  static String kotaBandung = 
      'https://kipi.covid19.go.id/api/get-faskes-vaksinasi?skip=0&province=JAWA+BARAT&city=KOTA+BANDUNG';
  static String covidBogor =
      'https://kipi.covid19.go.id/api/get-faskes-vaksinasi?skip=0&province=JAWA+BARAT&city=KAB.+BOGOR';
  static String kotaBogor = 
      'https://kipi.covid19.go.id/api/get-faskes-vaksinasi?skip=0&province=JAWA+BARAT&city=KOTA+BOGOR';
  static String kabCiamis = 
      'https://kipi.covid19.go.id/api/get-faskes-vaksinasi?skip=0&province=JAWA+BARAT&city=KAB.+CIAMIS';
}                                                                                                                                                                                                              